...
mr_output_file = "final_output_mr_q2.txt"
tez_output_file = "final_output_tez_q2.txt"
...
def create_mr_query_backup(query_num, extraConfs=[]):
    suffix = ""
    for conf in extraConfs:
        num = conf.split('=')[1]
        suffix += "_%s" % num

    output_file = os.path.join(output_dir, "query_mr.out")
    backup_file = "backup/query_mr_%d" % query_num + suffix + ".txt"
    os.system("mv %s %s" % (output_file, backup_file))
    ...
    for jhist in all_jhists:
        basename = os.path.basename(jhist)
        basename += suffix
        backup_file = "backup/%s" % basename
        os.system("mv %s %s" % (jhist, backup_file))

def create_tez_query_backup(query_num, extraConfs=[]):
    suffix = ""
    for conf in extraConfs:
        num = conf.split('=')[1]
        suffix += "_%s" % num

    output_file = os.path.join(output_dir, "query_tez.out")
    backup_file = "backup/query_tez_%d" % query_num + suffix + ".txt"
    os.system("mv %s %s" % (output_file, backup_file))
    ...
    hist = all_hists[0]
    backup_file = "backup/%s" % os.path.basename(hist) + suffix
    os.system("mv %s %s" % (hist, backup_file))
...
def run_mr_query(query_num, extraConfs=[]):
    ...
    for conf in extraConfs:
        cmd += "--hiveconf %s " % conf
    cmd += "-f %s --database tpcds_text_db_1_50) 2> %s" % (query_path,
                                                           query_output)
    #cmd = "(hive --hiveconf hive.execution.engine=mr "\
    ...
    results["extra_conf"] = []
    for conf in extraConfs:
        results["extra_conf"] += [conf]

    graph = draw_graph(job_stats)

    # Create backup of output files
    create_mr_query_backup(query_num, extraConfs)
...
def run_tez_query(query_num, extraConfs=[]):
    ...
    create_tez_query_backup(query_num, extraConfs)
    os.system("rm -rf output/history*")
...
def main():
    ...
    # Fine tune number of reducers for query 21
    query = 12
    best_reducers = 0
    best_parallel_copies = 0
    # Set to False if you want to skip this
    if True:
        best_result, best_reducers = 10000, 0
        for num_reducers in [1, 5, 10, 20]:
            extraConfs = []
            extraConfs += ["mapred.reduce.tasks=%d" % num_reducers]
            results, _, job_stats, graph = run_mr_query(query, extraConfs)
            print results
            write_mr_output(results, [], job_stats, graph)
            print "-" * 50
            print
            
            if results["run_time"] < best_result:
                best_result = results["run_time"]
                best_reducers = num_reducers

    # Fine tune the number of parallel transfers
    # Set to False if you want to skip this
    if True:
        # XXX Found value using previous results
        best_result, best_parallel_copies = 10000, 0
        for parallel_copies in [5, 10, 15, 20]:
            extraConfs = []
            extraConfs += ["mapred.reduce.tasks=%d" % best_reducers]
            extraConfs += ["mapreduce.reduce.shuffle.parallelcopies=%d" %
                           parallel_copies]
            results, _, job_stats, graph = run_mr_query(query, extraConfs)
            print results
            write_mr_output(results, [], job_stats, graph)
            ...
            if results["run_time"] < best_result:
                best_result = results["run_time"]
                best_parallel_copies = parallel_copies
            ...
            #print "-" * 50
            #print
        print "The best result was %s with %d parallel copies" % (best_result,
                                                                  best_parallel_copies)

    # Finetune the fraction of map tasks
    # Set to False if you want to skip this
    if True:
        # XXX Found value using previous results
        #best_reducers = 5
        #best_parallel_copies = 10
        best_result, best_fraction = 10000, 0
        for map_fraction in [0.05, 0.25, 0.5, 0.75, 1]:
            extraConfs = []
            extraConfs += ["mapred.reduce.tasks=%d" % best_reducers]
            extraConfs += ["mapreduce.reduce.shuffle.parallelcopies=%d" %
                           best_parallel_copies]
            extraConfs += ["mapreduce.job.reduce.slowstart.completedmaps=%s" %
                           map_fraction]
            results, _, job_stats, graph = run_mr_query(query, extraConfs)
            print results
            write_mr_output(results, [], job_stats, graph)
