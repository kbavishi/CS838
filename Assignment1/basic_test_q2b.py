...
def main():
    ...
    if True:
        ...
        for parallel_copies in [5, 10, 15, 20]:
            extraConfs = []
            extraConfs += ["tez.am.container.reuse.enabled=true"]
            extraConfs += ["tez.runtime.shuffle.parallel.copies=%d" %
                           parallel_copies]
            
            results, _ = run_tez_query(query, extraConfs)
            print results
            write_tez_output(results, [])
            print "-" * 50
            print
